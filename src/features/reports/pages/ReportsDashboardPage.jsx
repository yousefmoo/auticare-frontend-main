...
export default function ReportsDashboardPage() {
  const { data: reportsOverview, isLoading } = useQuery({
  ...
  return (
    <div className="fade-in space-y-6">
      <PageHeader
        title="Reports"
        subtitle="Track child progress, screening results, sessions summary, and statistics from one clean healthcare dashboard."
        action={
          <button
            type="button"
      ...
      {!isLoading && !reportsOverview && (
        <DashboardEmpty title="No reports yet" description="Reports and statistics will appear here once data is available." />
      )}
