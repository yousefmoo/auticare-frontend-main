...
export default function DoctorSessionsDashboard() {
  const { data: sessions = [], isLoading } = useQuery({
    queryKey: ['sessions', 'doctor'],
    queryFn: () => getSessions('doctor'),
  })
  ...
  return (
    <div className="fade-in space-y-6">
      <PageHeader
        title="Sessions"
        subtitle="مراجعة الجلسات القادمة وروابط الاجتماعات والحالة والملاحظات وملخص آخر جلسة لكل طفل."
      />

      <div className="grid gap-5">
        {isLoading && <DashboardSkeleton cards={2} />}
        {!isLoading && sessions.length === 0 && (
          <DashboardEmpty title="لا توجد جلسات" description="ستظهر هنا جلسات الطبيب القادمة عند توفرها." />
        )}
        {!isLoading && sessions.map((session) => (
          <SessionCard key={session.id} session={session} role="doctor" />
        ))}
      </div>
    </div>
  )
