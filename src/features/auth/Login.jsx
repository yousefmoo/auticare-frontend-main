...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // authStore.login handles parsing and normalization
      login(responseData, { remember: rememberMe })
      ...
      switch (userRole) {
        ...
        case USER_ROLES.PARENT: {
          const screeningResult = localStorage.getItem('auticare_screening_result')
          if (!screeningResult) {
      ...
      addToast({
        type: 'success',
        title: 'Welcome back',
        message: `Successfully signed in as ${userRole}`,
      })
    } catch (error) {
      // error.userMessage is set by the apiClient interceptor
      const errorMsg =
        error.userMessage ||
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-8 items-center">
        ...
        <div className="bg-[var(--card)] p-8 rounded-3xl shadow-xl border border-[var(--border)] slide-up">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold text-[var(--ink)]">Sign in</h2>
          ...
          <div className="mt-6 p-4 bg-[var(--card-alt)] border border-[var(--border)] rounded-lg">
            <p className="text-sm font-medium text-[var(--ink)] mb-2">Demo Accounts</p>
